...
class DatabaseManager:
    ...
    def search_by_id(self, id: str) -> List[Tuple[str, str, str]]:
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT ID, TYPE, VALUE FROM entries WHERE ID = ?", (id,)
                )
                return cursor.fetchall()
    ...
    def search_by_type(self, type: str) -> List[Tuple[str, str, str]]:
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT ID, TYPE, VALUE FROM entries WHERE TYPE = ?", (type,)
                )
                return cursor.fetchall()
    ...
    def search(
        self, search_term: str, search_by: str = "id"
...
def search(
    search_term: str, db_path: str, search_by: str = "id"
) -> List[Tuple[str, str, str]]:
    db = DatabaseManager(db_path)
