...
if st.session_state["add_mode"]:
    ...
    method = st.selectbox(
        "Method", ["Cloning an existing group", "From .csv", "Manually"]
    )
    st.write(f"Selected method: {method}")

    if method == "Manually":  # Manual input
        names = st.text_input(
            "Add the student names separated by a comma (ex. Peter, John, Mark...)"
        )
        ...
        if st.session_state["df_set"] == True:
            st.session_state["temp_df"] = st.data_editor(
                ...
                key="data_editor",
                disabled=("Reports", "Exams", "Finals", "Selfs", "Sum"),
            )
        ...
        if to_clone:
            ...
            if confirm_clone:
                ...
                if not csv_path.exists():
                    ...
                    create_group(clone_df["Students"].tolist()).to_csv(
...
if st.session_state["modify_mode"]:
    ...
    if group_name:
        action = st.selectbox(
            "What to do with the group?", ["Edit", "Archive", "Delete"]
        )
        ...
        if action == "Edit":
            ...
            st.write(
                f"Exported report count: {export_count}"
                if export_count > 0
                else "No exports from this group have been generated"
            )

            user_sure = st.toggle(
                f"I am sure I want to {action.lower()} the group {group_name}"
            )
